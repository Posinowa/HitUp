...
**Adding a kind of exercise** is one class implementing `ExerciseRenderer` and one line in `exerciseRendererRegistryProvider`. A type with no renderer yet falls back to the plain one, which adds nothing to the title and instructions, so a day still runs while its renderers land. Today `text`, `timer`, `rive`, `articulation`, `tongueTwister` and `letter` have theirs; `rive` and `articulation` share `RiveExerciseRenderer` (`RIVE.md`).
